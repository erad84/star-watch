/* Adapted from the official Pebble Compass app (coredevices/pebble-compass). */
...
typedef struct {
    ...
    CompassCalibrationWindowHandler filled_handler;
    bool filled_notified;
...
#ifdef PBL_ROUND
static const int CALIBRATION_WINDOW_RING_MARGIN = 18;
#else
...
static bool ring_is_fully_filled(const CompassCalibrationWindowData *data) {
    for (int i = 0; i < CALIBRATION_NUM_SEGMENTS; i++) {
        if (data->segment_value[i] < CALIBRATION_THRESHOLD_FILLED) {
            return false;
        }
...
void compass_calibration_window_merge_value(CompassCalibrationWindow *window, int32_t angle, uint8_t intensity) {
    ...
    if (!data->filled_notified && data->filled_handler && ring_is_fully_filled(data)) {
        data->filled_notified = true;
        data->filled_handler(window);
    }
}
...
void compass_calibration_window_set_influenced_by_magnetic_interference(CompassCalibrationWindow *window, bool influenced) {
    ...
    reset_segment_data(data, false);
    update_description_if_needed(data);
}

void compass_calibration_window_reset(CompassCalibrationWindow *window) {
...
int compass_calibration_window_fill_percent(CompassCalibrationWindow *window) {
    CompassCalibrationWindowData *data;
    int i;
    int sum = 0;
    if (!window) {
        return 0;
    }
    data = window_get_user_data((Window *)window);
    if (!data) {
        return 0;
    }
    for (i = 0; i < CALIBRATION_NUM_SEGMENTS; i++) {
        int v = data->segment_value[i];
        if (v > CALIBRATION_THRESHOLD_FILLED) {
            v = CALIBRATION_THRESHOLD_FILLED;
        }
        sum += v;
    }
    return (sum * 100) / (CALIBRATION_NUM_SEGMENTS * CALIBRATION_THRESHOLD_FILLED);
}
...
void compass_calibration_window_set_filled_handler(CompassCalibrationWindow *window,
        CompassCalibrationWindowHandler filled_handler) {
    CompassCalibrationWindowData *data = window_get_user_data((Window*)window);
    data->filled_handler = filled_handler;
}
...
#ifdef PBL_ROUND
    GRect all_text_rect = (GRect){.size = GSize(140, 70)};
#else
...
static void _gpath_draw_filled(GContext* ctx, GPath* path) {
    ...
    min_y = MAX(min_y, 0);
    max_y = MIN(max_y, PBL_DISPLAY_HEIGHT - 1);
